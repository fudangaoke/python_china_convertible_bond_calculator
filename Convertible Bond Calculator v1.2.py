...
def getQuote(ticker):
	return float(ts.get_realtime_quotes(str(ticker))['price'][0])
def getName(ticker):
	return ts.get_realtime_quotes(str(ticker))['name'][0]
...
class CBPlot:
	...
	def plot_cb(self):
		simulated_price_series=self.generateStockPriceSeries(upper=max(1.6,self.cb.ConvertValue(self.stock_price)/self.cb.face_value))
		x_convert_value=[]
		y_cb_total_value=[]
		lower_bound=[]
		...
		plt.grid(ls='--')
		plt.xticks(np.arange(30,max(170,self.cb.ConvertValue(self.stock_price)),10))
		plt.yticks(np.arange(80,max(170,max(170,self.cb_spot_price)),10))
		plt.title('Convertible Bond Analysis ('+str(self.cb_name)+')',fontproperties='SimHei',fontsize=20)
		plt.xlabel('Conversion Value')
		plt.ylabel('Convertible Bond Spot Price')
		plt.legend([curve_1,curve_2,point_1],['Black Scholes Value (Interest rate = '+percent(self.interest_rate)+", Volatility = "+percent(self.volatility)+")",'Lower Bound',"CB Spot Price (Conversion Value = "+str(self.cb.ConvertValue(self.stock_price))+', Spot Price = '+str(self.cb_spot_price)+')'],loc='upper left')
		plt.show()
	...
	def __repr__(self):
		...
		string+="\n********************************\n\n"
		string+="Above Bond Floor rate （债底溢价率）\n= ((CB Spot Price / Bond Floor Value)-1)*100%\n"
		string+="\nConvertion rate (转股溢价率) \n= ((CB Spot Price / Conversion Value)-1)*100%\n\n"
		string+="Black Scholes Discount rate (理论折价率) \n= - ((Black Scholes Value / CB Spot Price)-1)*100%\n"
		string+="\n********************************"
		return string
...
class GUI(QWidget):
	...
	def initialize_UI(self):
		...
		self.setWindowTitle("Covertible Bond Calculator v1.2 (by 13300200009@fudan.edu.cn)")
		
		self.label1=QLabel("CB Ticker",self)
		self.label1.move(20,60)
		self.text1=QLineEdit("113021",self)
		self.text1.move(120,55)

		self.label2=QLabel("Stock Ticker",self)
		self.label2.move(20,90)
		self.text2=QLineEdit("601998",self)
		self.text2.move(120,85)

		self.label3=QLabel("Coupon List",self)
		self.label3.move(20,120)
		self.text3=QLineEdit("[0.3,0.8,1.5,2.3,3.2,111]",self)
		self.text3.move(120,115)
		self.text3.setFixedWidth(170)

		self.label4=QLabel("Maturity Date",self)
		self.label4.move(20,150)
		self.text4=QLineEdit("[2025,3,3]",self)
		self.text4.move(120,145)

		self.label5=QLabel("Conversion Price",self)
		self.label5.move(20,180)
		self.text5=QLineEdit("7.45",self)
		self.text5.move(120,175)

		self.label6=QLabel("Interest Rate",self)
		...
		self.label9=QLabel("2. Support real time quotation.",self)
		self.label9.move(20,370)
		self.label9=QLabel("3. Copyright 13300200009@fudan.edu.cn \n   All rights reserved.",self)
		self.label9.move(20,400)
